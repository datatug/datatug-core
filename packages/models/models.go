...
// ColumnProps holds column metadata
...
// Validate return error if not valid
func (v ColumnProps) Validate() error {
	if v.Name == "" {
		return validation.NewErrRecordIsMissingRequiredField("name")
	}
...
// Column holds col metadata
type Column struct {
	ChangeType ChangeType `json:"-"`
	ColumnProps
	ByEnv       map[string]Column `json:"byEnv,omitempty"`
...
func (v *ColumnModel) Validate() error {
	if err := v.Column.Validate(); err != nil {
		return err
	}
...
// Validate return error if not valid
func (v Column) Validate() error {
	if err := v.ColumnProps.Validate(); err != nil {
		if v.Name == "" {
			return err
		}
