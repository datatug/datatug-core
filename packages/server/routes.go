...
func initRouter() {
	...
	handlerFunc(http.MethodGet, "/datatug/queries/all_queries", endpoints.GetQueries)
	handlerFunc(http.MethodGet, "/datatug/queries/get_query", endpoints.GetQuery)
	handlerFunc(http.MethodPut, "/datatug/queries/create_folder", endpoints.CreateQueryFolder)
	handlerFunc(http.MethodPost, "/datatug/queries/create_query", endpoints.CreateQuery)
	handlerFunc(http.MethodPut, "/datatug/queries/update_query", endpoints.UpdateQuery)
	handlerFunc(http.MethodDelete, "/datatug/queries/delete_query", endpoints.DeleteQuery)
	handlerFunc(http.MethodDelete, "/datatug/queries/delete_folder", endpoints.DeleteQueryFolder)

	handlerFunc(http.MethodGet, "/datatug/boards/board", endpoints.GetBoard)
	handlerFunc(http.MethodPost, "/datatug/boards/create_board", endpoints.CreateBoard)
	handlerFunc(http.MethodPut, "/datatug/boards/save_board", endpoints.SaveBoard)
	handlerFunc(http.MethodDelete, "/datatug/boards/delete_board", endpoints.DeleteBoard)

	handlerFunc(http.MethodGet, "/datatug/data/recordsets_summary", endpoints.GetRecordsetsSummary)
	handlerFunc(http.MethodGet, "/datatug/data/recordset_definition", endpoints.GetRecordsetDefinition)
	handlerFunc(http.MethodGet, "/datatug/data/recordset_data", endpoints.GetRecordsetData)
	handlerFunc(http.MethodPost, "/datatug/data/recordset_add_rows", endpoints.AddRowsToRecordset)
	handlerFunc(http.MethodPut, "/datatug/data/recordset_update_rows", endpoints.UpdateRowsInRecordset)
	handlerFunc(http.MethodDelete, "/datatug/data/recordset_delete_rows", endpoints.DeleteRowsFromRecordset)
}
